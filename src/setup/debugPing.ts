...
export const debugPing = (): void => {
  ...
  const inventory = NotionClient.queryDatabase(CONFIG.NOTION_INVENTORY_DB_ID, { page_size: 1 });
  logInfo('debugPing', `3/5 在庫DB接続: OK (${inventory.results.length} page)`);

  const users = NotionClient.queryDatabase(CONFIG.NOTION_USERS_DB_ID, { page_size: 1 });
  logInfo('debugPing', `4/5 ユーザーDB接続: OK (${users.results.length} page)`);

  const purchases = NotionClient.queryDatabase(CONFIG.NOTION_PURCHASES_DB_ID, { page_size: 1 });

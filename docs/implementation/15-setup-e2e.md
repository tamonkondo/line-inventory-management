...
| 23 | 「新規登録」→品名→カテゴリ選択→購入先を2つ選択→決定 | Notionの品目にカテゴリ・購入先が入っている(実装書17) |
| 24 | 「新規 米」→カテゴリをスキップ→購入先を「決定」のみ | 品名だけの品目ができる(実装書17) |
| 25 | Notionでカテゴリの選択肢を追加→10分後に新規登録 | 新しい選択肢がボタンに出る(実装書17) |
